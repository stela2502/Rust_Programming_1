...
This allows for the match to check if an error has occured using the two functions Ok and Err that both take a Result as input:
...
But you have not even seen most of it as what you see here first defines a data variable using the Data::read_file() function and then uses the Data::print() function to print the contens of the data object. Therfore the real code loading the data is hidden in the library.
...
First this function takes ownership of the data using the into_iter() function. This means that we can no longer access data in the main function. This is part of Rusts more secure memory management: Only one function can have access to the data.

The Self{} statement creates the object from it's parts. Here we have the rows, cols, the rownames from the function call and the data array. 
...
Adding more 'space' to a variable is quite costy. Hence here we iterate over the data twice. Lines 32..52 read the data and 'only' collect the column and row counts.
...
Translates to: split the line by 'sep' and give me ownership of the variables you create as Vec<&str>; iterate over the variables in the vector that I want to be able to modify.
...
The only other interesting part here is the ``let v = match val.parse::<f64>() {``. This tries to convert the data into a f64 and if that fails it converts it to a usize. And if both ties fail it throws an error, but does not break. It returns 0.0 (a f64) by default.
